...
#' 
#' @return An object of type "stabest2" 
#' 
#' @export
#' 
#' @import Matrix
...
stabest2.default <- function(student.prefs, 
  ...
  # cargo los datos de schoolmarket200 para ir probando el código
  # creo Vs y Vs_min 
  #schoolmarket200 <- schoolmarket200 %>% 
  #  group_by(sch_id) %>% 
  #  mutate(Vs_ = max(sch_rk_true) + 1 - sch_rk_true) 
  #
  #Vs_min <- schoolmarket200 %>% 
  #  filter(sch_assignment==1) %>% 
  #  group_by(sch_id,sch_assignment) %>% 
  #  summarise(Vs_min_ = min(Vs_)) %>% 
  #  select(-sch_assignment)
  # 
  #schoolmarket200 <- left_join(schoolmarket200,Vs_min,by="sch_id")
  #  
  #data <- schoolmarket200
  #student.prefs <- choice_rk~-1 + distance + stu_score:sch_mscore + sch_FE
  ...
  data$use <- F; data$use[match(rownames(Xc), rownames(data))] <- T
  gc()
  ...
  res = stabest2_internal(Vc=data$Vc_,
                          Vs=data$Vs_,
                          Vs_min=data$Vs_min_,
                          Xc=Xc, 
                          XcXcInv=XcXcInv, 
